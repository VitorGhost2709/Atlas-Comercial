...
export function CompaniesPage() {
  ...
  const isClinics = listing?.variant === 'clinics'
  const rowsGrouped = listing?.variant === 'grouped' ? listing.rows : []
  const rowsClinics = listing?.variant === 'clinics' ? listing.rows : []
  const listaVazia =
    !loading && !error && listing && (isClinics ? rowsClinics.length === 0 : rowsGrouped.length === 0)
      ...
      <div className="rounded-2xl border border-white/10 bg-[#2d1f44]/35 shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur">
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-5 py-4">
          <div className="text-sm font-medium text-zinc-100">
            {isClinics ? 'Lista de clínicas' : 'Lista de empresas'}
          </div>
          ...
          {!idsValidos ? (
            ...
            <ul className="grid gap-3 p-2 sm:p-3">
              {rowsClinics.map((loja) => (
                <li key={loja.id}>
                  <Link
                    to={storesBaseRoute ? storesBaseRoute(loja.empresa_id) : '#'}
